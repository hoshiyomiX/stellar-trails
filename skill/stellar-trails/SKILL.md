...
if [ -f /home/z/my-project/upload/PAT ]; then
  ...
  if [ -n "$_OWNER_LOGIN" ]; then
    ...
    # v9.18.0: Uses GIT_AUTHOR_EMAIL env var
    # v9.18.0: Uses GIT_AUTHOR_NAME env var
...
if [ -z "$CURRENT" ]; then echo "✗ Block A Step 3 FAILED: could not read current version"; exit 1
  ...
  # v9.18.0: Conditional force update — check scan verdict before updating
  ...
  if [ "$REG_MOD" = "clean" ]; then
    echo "⚠️ Block A Step 3: DRIFT — local v$CURRENT vs registry v$LATEST (moderation: clean) — FORCE UPDATING..."
    ...
    echo "✓ Block A Step 3: FORCE UPDATE CONFIRMED — local v$POST_VERSION = registry v$LATEST (scan: clean)"
